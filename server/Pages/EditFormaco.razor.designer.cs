...
namespace C4G.Pages
{
    public partial class EditFormacoComponent : ComponentBase
    {
        ...
        {
            var c4GGetFormacoByidRecursosResult = await C4G.GetFormacoByidRecursos(id_recursos);
            formaco = c4GGetFormacoByidRecursosResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(C4G.Models.C4G.Formaco args)
        {
            try
            {
                var c4GUpdateFormacoResult = await C4G.UpdateFormaco(id_recursos, formaco);
                DialogService.Close(formaco);
            }
            catch (Exception c4GUpdateFormacoException)
            {
                    NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to update Formaco");
            }
        }
